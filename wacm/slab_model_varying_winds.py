...
class synthetic_wacm:
    ...
    def get_wacm_nio(self,t0,t1,t_out=None,use_hourly_wind=False):
        ...
        up,vp,param=reconstruct_NIO_short_segment(u,v,uw,vw,self.lat0,t_out=t_out)
...
def remove_tides(d,lat0):
    ...
    cc=utide.solve(tt,u.values,v.values,lat=lat0,verbose=False)
...
def reconstruct_NIO_short_segment(uobs,vobs,uwobs,vwobs,lat0,
    ...
    u_pred,v_pred,param=optimize_slab_noshear_withtide(
                                         wacm_u,wacm_v,
                                         wacm_uw,wacm_vw,
                                         f0,t_out=t_out,has_tides=has_tides)
    u_pred=xr.DataArray(u_pred,dims=('time'),coords={'time':t_out})
    v_pred=xr.DataArray(v_pred,dims=('time'),coords={'time':t_out})
            
    return u_pred, v_pred, param
...
def predict_slab_noshear(x,tt,t_tau_days,taux,tauy,f): 
    """
        
    x: the parameter space [u0,v0,H,c,u_bias,v_bias, taux, tauy]
    
    tt: array of time in days referenced to 1979-01-01 00:00:00 for the evaluation point of the integration
    
    use the parameters in x, integrate a slab model forward, compare to the truth giving in y
    
    t_tau_days: array of time in days referenced to 1979-01-01 00:00:00 for taux and  tauy
    
    taux,tauy : numpy array, wind stress on t_tau_days axis 
    ...
    n=t_tau_days.size
    taux_func=interp1d(t_tau_days,x[6:6+n]+taux,fill_value="extrapolate")
    tauy_func=interp1d(t_tau_days,x[6+n:]+tauy,fill_value="extrapolate")
    ...
    sol = solve_ivp(slab_noshear, [tt.min(), tt.max()], [u0,v0], t_eval=tt, args=(f,H,c,taux_func,tauy_func)) #(1e-4, 10.0, 1e-5, taux,tauy,0.,0.,0.,0.) )
...
def loss_slab_noshear(x,y,tt,
                      t_tau_days,taux,tauy,
                      f,T=[],has_tides=False,
                      use_wind_as_control=True): 
    """
    the loss function
    
    x: the parameter space [u0,v0,H,c,u_bias,v_bias, taux_delta, tauy_delta]
    
    use the parameters in x, integrate a slab model forward, compare to the truth giving in y
    
    y: observations, y[:n/2]=u and y[n/2:]=v
    
    tt: time axis in days (referenced to 1979-01-01 00:00:00) 
    
    t_tau_days: time axis in days for tau referenced to 1979-01-01 00:00:00
    ...
    u,v=predict_slab_noshear(x,tt,t_tau_days,taux,tauy,f)
    ...
    #win=np.r_[win,win].flatten()
    #x=np.linspace(-np.pi/2+np.pi/20,np.pi/2-np.pi/20,u.size)
    #weight=np.cos(x)**0.5 
    n=taux.size
    taux=x[6:6+n]
    tauy=x[6+n:]
    loss=(np.r_[u, v].flatten() - y) #, taux/10, tauy/10 ].flatten() #*np.r_[weight,weight]
...
def optimize_slab_noshear_withtide(u,v,taux,tauy,f0,t_out,
                                   has_tides=False,
    ...
    if use_wind_as_control:
        x0=x0+(2*len(taux))*[0]
        
    uv_truth=np.r_[u.values,v.values].flatten()
    ...
    t_tau_days=(taux.time.values-np.datetime64('1979-01-01 00:00:00'))/np.timedelta64(1,'D') 
    t_uv_days=(u.time.values-np.datetime64('1979-01-01 00:00:00'))/np.timedelta64(1,'D') 
    ...
    res_lsq = least_squares(loss_slab_noshear, x0, loss='cauchy',
                        args=(uv_truth, t_uv_days, taux.values, 
                              tauy.values, t_tau_days, f0, T_tide,has_tides,
                              use_wind_as_control))
    ...
    u_pred,v_pred = predict_slab_noshear(res_lsq.x,t_out_days,t_tau_days,taux.values,tauy.values,f0)
    ...
    return u_pred,v_pred, {'H':res_lsq.x[2],'c':res_lsq.x[3]}

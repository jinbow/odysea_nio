...
def glue_bin_to_h5(fns,fnout,varn,datashape,nrec=None,dtype='>f4'):
    import numpy as np
    if nrec==None:
        nrec=len(fns)
    fns=sorted(fns)
    for irec,fn in enumerate(fns):
        print('read and save ',fn)
        d=np.fromfile(fn,dtype=dtype).reshape(datashape)
        saveh5(fnout,varn,d,dtype=dtype,nrec=nrec,irec=irec)
        del d
    return


def saveh5(filename, datasetname, data, dtype=None, nrec=None, 
           irec=None, driver=None,description=None,message=False):
    
    import h5py,sys
    
    with h5py.File(filename,driver=driver) as hfile:
        if message:
            print("save data to HDF5, data size=",datasetname,data.shape)
        
        if (nrec==None) != (irec==None):
            print("Warning ... nrec and irec should be specified at the same time")
            print("Usage: saveh5(filename, datasetname, data, nrec, irec)")
            sys.exit(0)
            
        if nrec != None:
            shapes = (nrec,)+data.shape 
        else:
            shapes= data.shape
        ...
        dst = hfile.require_dataset(datasetname, shape=shapes, dtype=dtype)

        if description != None:
            dst.attrs['description']=description
...
def saveh5s(filename,datagroup):
    for key in datagroup.keys():
        saveh5(filename,key,datagroup[key])
    return

def loadh5(filename, datasetname=None):
    import h5py,sys
    ...
    if datasetname==None:
        ...
        try:
            data = hfile[datasetname][...]
            print("hfile keys",hfile.keys())
            print("load data from %s, data size="%(datasetname), data.shape)
            del hfile
        except:
            print("error in loading HDF5 file "+filename)
            print("variables in the file are ", hfile.keys())
            sys.exit()

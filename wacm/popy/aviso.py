...
updated on July 12, 2019 for Pleiades @JPL

'''

pth='/nobackup/jwang23/data/copernicus/my.cmems-du.eu/Core/'
pth_l3=pth+'SEALEVEL_GLO_PHY_L3_REP_OBSERVATIONS_008_045/'
pth_l4=pth+'SEALEVEL_GLO_PHY_L4_REP_OBSERVATIONS_008_047/dataset-duacs-rep-global-merged-allsat-phy-l4/'
...
class aviso:
    ...
    def __init__(self):
        import numpy as np
        from popy import links
        pths=links.paths()
        self.root = pths.aviso
        ...
        self.lat = np.arange(721) * 0.25 - 90.0+0.125
    ...
    def FileName(self, YMD, vname='h'):
        ...
        pths=self.root+YMD[:4]+'/'+'dt_global_allsat_madt_h_%s_20140106.nc'%YMD
        return pths
    ...
    def loadgrid(self):
        ...
        if os.path.exists(fnpkl):
            ...
            fn = self.root + '1994/dt_global_allsat_madt_h_19940101_20140106.nc' 
            f = Dataset(fn, 'r').variables
            lat, lon = f['lat'][:], f['lon'][:]
            #date = self.timestamps()
            g = {'lat':lat, 'lon':lon, 'date':f['time']}
            #pickle.dump(g, open(fnpkl, 'wb'))
            return g
        
    def LoadData(self, YMD='19930101', subdomain=[], index=True):

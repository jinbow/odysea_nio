...
def assemble_grid(folder_name):
    """assemble grid data contained in the folder "folder_name" into one h5 file."""
    import glob,h5py,popy,os
    if folder_name[-1] !='/':
        folder_name+='/'
        
    grid_folder=folder_name+'grid/'
    popy.utils.checkmake(grid_folder)
    fns = glob.glob(folder_name+'*.001.001.data')
    varns=[]
    for fn in fns:
        varns.append(fn[:-13].split('/')[-1])
        
    varns.extend(['DRC','DRF','PHrefC','RC','RF','RhoRef'])
    h5fn = folder_name+'grid.h5'
    with h5py.File(h5fn) as h5f:
        for varn in varns:
            dat = rdmds(folder_name+varn)
            print varn
            datasetname='/%s'%(varn)
            dst = h5f.require_dataset(datasetname, 
                                        shape=dat.shape, 
                                        dtype='>f4')
            dst[...]=dat[...].astype('>f4')
            os.popen('mv %s* %s'%(varn,grid_folder))
    print "saved grid data to %s \n and moved .data and .meta files into %s"%(h5fn,grid_folder)
